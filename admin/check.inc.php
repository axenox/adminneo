...
if ($row) {
	if (DIALECT == "sqlite") {
		...
		if (!$row["drop"]) {
			$success = (bool)queries("ALTER TABLE " . table($TABLE) . " ADD" . ($row["name"] != "" ? " CONSTRAINT " . idf_escape($row["name"]) : "") . " CHECK ($row[clause])"); // TODO SQL injection
		}
	}
	queries_redirect(
...
if ($name != "") {
	page_header(lang('Alter check') . ": " . h($name), ["table" => $TABLE, lang('Alter check')]);
} else {
	page_header(lang('Create check') . ": " . h($TABLE), ["table" => $TABLE, lang('Create check')]);
}
